...
#include <unistd.h>
#include <filesystem>
#include <unistd.h> // to check working directory
...
int main()
{
...
#endif
    int nc;
    const std::string imageP = parentPath + "/resources/leonardo-1.jpg";
    imagePath = imageP.c_str();
    unsigned char *data = stbi_load(imagePath, &SCR_WIDTH, &SCR_HEIGHT, &nc, 0);

    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Nanquim", NULL, NULL);
    if (window == NULL)
    {
    ...
    const std::string imageT1 = parentPath + "/resources/GrayNoiseMedium.png";
    unsigned int imageTexture1 = loadTexture(imageT1.c_str());
    const std::string imageT2 = parentPath + "/resources/pebbles.png";
    unsigned int imageTexture2 = loadTexture(imageT2.c_str());
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        if(shouldDraw)
        {
            ...
            if(past>0.1)
            {
                ...
                sprintf(title_string, "Nanquim - FPS = %.i ", (int)fps);
